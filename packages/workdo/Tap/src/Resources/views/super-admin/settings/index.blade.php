            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" name="company_tap_secret_key" id="company_tap_secret_key"
                        class="form-control" value="{{ !isset($settings['company_tap_secret_key']) || is_null($settings['company_tap_secret_key']) ? '' : $settings['company_tap_secret_key'] }}"
                        placeholder="{{ __('Secret Key') }}" {{ isset($settings['tap_payment_is_on']) && $settings['tap_payment_is_on'] == 'on' ? '' : ' disabled' }}>
                </div>
            </div>
        </div>
    </div>
